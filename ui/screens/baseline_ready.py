...
class BaselineReadyScreen(tk.Frame):
    def __init__(self, parent, app):
        ...
        if self.app.lang == "ar":
            title = rtl("تم قياس خط الأساس")
            body = rtl("قم بتحضير محلول البنسلين جي والمحقنة.\n\nاضغط التالي للمتابعة.")
            btn = rtl("التالي")
            btn_font = FONTS.get("button_ar", FONTS["button"])
        else:
            title = "Baseline Complete"
            body = "Have the Penicillin G solution and syringe ready.\n\nPress NEXT to continue."
            btn = "NEXT"
        ...
        tk.Button(
            ...
            width=16,
            height=2,
            command=lambda: self.app.show("add_peng"),
